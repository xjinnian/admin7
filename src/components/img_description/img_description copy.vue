  ...
  <q-card class="my-card mw50vw q-px-md w50vw">
    ...
    <q-card-section class="q-py-md q-gutter-y-md">
      ...
      <q-tab-panels v-model="current_tab" animated>
        <q-tab-panel v-for="(v, i) in all_tabs" :key="i" :name="i">
          <div class="row">
    ...
    <q-card-actions align="right">
      <div>{{ current_tab + 1 }}</div>
      <q-btn
        color="primary"
        @click="handle_conform_edit"
        :label="`保存第${current_tab + 1}条数据`"
      />
      <q-btn
        v-close-popup
        color="deep-orange"
...
export default {
  ...
  methods: {
    ...
    async handle_conform_edit() {
      await api_imgDescription.updateImgDescsApi(this.all_tabs);
