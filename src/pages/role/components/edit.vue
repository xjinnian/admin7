<template>
  <q-card class="my-card mw50vw q-px-md w50vw">
    <q-card-section class="no-padding">
      <div class="row">
        <div class="text-weight-bolder lh40">
          {{ editing_obj.id ? "编辑" : "新建" }}角色
        </div>
        <q-space />
        <q-btn flat round icon="close" v-close-popup />
      </div>
    </q-card-section>
    <q-separator />
    <q-card-section class="q-py-md q-gutter-y-md">
      <div class="row">
        <div class="w120 form-item-label">角色名</div>
        <q-input class="col" filled dense v-model="editing_obj.name" />
      </div>

      <div class="row">
        <div class="w120 form-item-label">菜单</div>
        <FilterSelect
          class="col"
          v-model="editing_obj.menus"
          label="多选菜单/可搜索"
          color="teal"
          optionValue="name"
          :multiple="true"
          :options="menuConfig"></FilterSelect>
        <q-btn style="max-height:50px" color="primary" label="全选" @click="handle_menus"></q-btn>
      </div>


      <div class="row">
        <div class="w120 form-item-label">备注</div>
        <q-input
          class="col"
          filled
          dense
          type="textarea"
          v-model="editing_obj.mark"
        />
      </div>
    </q-card-section>
    <q-separator />
    <q-card-actions align="right">
      <q-btn color="primary" @click="handle_conform_edit" label="确定" />
      <q-btn
        v-close-popup
        color="deep-orange"
        glossy
        @click="handle_cancel_edit"
        label="取消"
      />
    </q-card-actions>
  </q-card>
</template>
<script>
import { ref } from "vue";
import menuConfig from "src/config/menu.js";
import FilterSelect from 'src/components/filter-select';
export default {
  emits: ["emit-edit"],
  components: {
    FilterSelect
  },
  data() {
    return {
      editorRef: ref(null),
      files: [],
      editing_obj: {},
      temp_obj: {
        name: "",
        mark: "",
        menus:[]
      },
      menuConfig,
    };
  },
  props: {
    detail_obj: {},
  },
  created() {
    if (this.detail_obj.id) {
      console.log(this.detail_obj);
      this.editing_obj = Object.assign(
        this.$lodash.cloneDeep(this.temp_obj),
        this.$lodash.cloneDeep(this.detail_obj)
      );
      console.log(this.editing_obj);
    } else {
      this.editing_obj = this.$lodash.cloneDeep(this.temp_obj);
    }
  },
  methods: {
    handle_menus(){
      this.editing_obj.menus = this.menuConfig.map(i=>i.name)
    },
    /**
     * 确定 提交更改
     *
     */
    handle_conform_edit() {
      let { id, name, mark, menus } = this.editing_obj;
      let final_obj = {
        id,
        name,
        mark,
        menus
      };
      if (!name) {
        this.$q.notify({
          type: "negative",
          message: "名字必填",
        });
        return
      }
      this.$utils.remove_false_key(final_obj);
      this.$emit("emit-edit", final_obj);
    },
    /**
     * 取消  取消更改
     *
     */
    handle_cancel_edit() {},
  },
};
</script>
<style lang="scss" scoped></style>
