  ...
  <div class="q-pa-md">
    <div ref="tableHeader">
      ...
      <div class="row q-my-sm q-gutter-sm" v-if="show_search_form_area">
        ...
        <q-input outlined clearable v-model="search_form.value" label="默认值" dense class="w200 q-mr-md"
          @keyup.enter="hadle_click_search" />
        <q-input outlined clearable v-model="search_form.operator" label="最后操作人" dense class="w200 q-mr-md"
          @keyup.enter="hadle_click_search" />
        ...
        <q-btn color="purple" glossy class="q-mr-md" :loading="flush_imgUrl_loading" @click="hadle_flush_imgUrl"
          label="刷新图片地址" />
        <q-btn color="purple" glossy class="q-mr-md" :loading="flush_imgdesc_loading" @click="hadle_flush_imgdesc"
          label="刷新图片描述统计" />
    ...
    <q-table class="sticky-header-table" :rows="table_data" :columns="columns" row-key="name"
      ...
      <template v-slot:body-cell-value="props">
        <q-td :props="props">
          <img :src="$config.get_cdn_url(props.row.value)" height="40" class="thumbnail_image q-ml-md" />
        </q-td>
      </template>
    ...
    <q-dialog v-model="show_dialog_img_description" persistent>
      <ImgDescription :detail_obj="detail_obj" :key_type="KEY_TYPE_ASSETS_KEY" @emit-edit="handle_conform_edit"
...
import { api_assetsKey,api_imgDescription, api_keyGroup } from 'src/api/index.js'
...
import { KEY_TYPE_ASSETS_KEY } from 'src/config/key_type'
...
export default {
  ...
  computed: {
    ...
    // ...mapGetters(['get_all_keygroup_assets'])
  },
  watch: {
    search_form_group_data(group) {
      this.search_form.group = group?.key

      if (group) this.search_form.project = group.project
    },
  },
  created() {
    this.get_group_list()
  ...
  methods: {
    // ...mapActions(['set_all_keygroup']),
    async get_group_list(){
      const { data } = await api_keyGroup.handle_read({
        key_type: KEY_TYPE_ASSETS_KEY,
      })
      const docs = data?.data?.docs || []
      const asset_level_map = this.$config.asset_level_map
      this.attribuits_options = docs.map(item=>{
        let label = this.project_map[item.project] + ' / ' + asset_level_map[item.level] + ' / ' + item.names?.zh_cn
        return {
          ...item,
          label,
          value: item.id
    ...
    async hadle_flush_imgdesc() {
      ...
      try {
        await api_imgDescription.handle_flush({key_type: KEY_TYPE_ASSETS_KEY})
      } catch (error) { }
      this.$lodash.delay(() => {
    ...
    async hadle_flush_imgUrl() {
      ...
      try {
        const { data } = await api_assetsKey.handle_flush_imgUrl()
