...
import { api_file, api_b_sdk_menu } from "src/api/index";
...
export default {
  ...
  computed: {
    ...mapGetters(["get_all_language", "get_all_b_sdk_documdent"]),
  },
  created() {
    this.related_doc_options = markRaw(
      this.get_all_b_sdk_documdent.filter((item) => item.status == 1)
    );
  ...
  methods: {
    async get_info() {
      ...
      let {
        data: { data },
      } = await api_b_sdk_menu.handle_info(params);
      console.log("==========", data);
    ...
    filterFn(val, update, abort) {
      let get_all_b_sdk_documdent = this.get_all_b_sdk_documdent;
      update(() => {
        const needle = val.toLowerCase();
        console.log(" this.get_all_b_sdk_documdent-", this.get_all_b_sdk_documdent);
        let arr = get_all_b_sdk_documdent.filter(
