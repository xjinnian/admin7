...
import { api_docpool as api_namespace, api_mddocpool } from "src/api/index";
...
export default {
  ...
  created() {
    console.error("编辑页面创建 -------", this.detail_obj);
  ...
  methods: {
    ...
    async get_mddoc_history_by_language() {
      if (this.detail_obj.id) {
        let res = await api_mddocpool.handle_read({
     ...
     */
    handle_conform_edit() {
      let { id, mark, name_zh, name_en, status, topic, url } = this.editing_obj;
      let final_obj = {
        id,
        mark,
        name_zh,
    ...
    handle_file_emit_upload_file_path(obj) {
      ...
      api_mddocpool.handle_create(params).then((res) => {
        this.get_mddoc_history_by_language();
      });
    },
    /**
    ...
    async handle_historty_list_item_update(record,key,updateHook) {
      let res = await api_mddocpool.handle_update({
        id: record.id,
        [key]: record[key]
      })
