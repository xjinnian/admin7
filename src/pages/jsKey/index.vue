 ...
 * @FilePath: /client-api-doc-admin/src/pages/jsKey/index.vue
 * @description    : JSkey
-->
<template>
  <div class="q-pa-md">
    ...
    <!-- 编辑弹窗  图片 录入的 -->
    <q-dialog v-model="dialog_js_img_description" persistent>
      <ImgDescription :detail_obj="detail_obj" :key_type="KEY_TYPE_JS_KEY" @emit-edit="handle_conform_edit"
        @closeDialog="() => (dialog_js_img_description = false)" />
    </q-dialog>
...
import { api_jsKey,api_imgDescription, api_keyGroup } from "src/api/index.js";
...
import {KEY_TYPE_JS_KEY} from 'src/config/key_type.js'
...
export default {
  ...
  data() {
    return {
      attribuits_options:[],// 属性组列表
      KEY_TYPE_JS_KEY,
      //API 命名空间
      api_namespace: api_jsKey,
      //列表 表格项配置
      columns,
      //编辑弹窗内的数据
      detail_obj: {
  ...
  methods: {
    // ...mapActions(["set_all_keygroup"]),
     ...
     async get_group_list(){
      const { data } = await api_keyGroup.handle_read({
        key_type: KEY_TYPE_JS_KEY,
      })
      const docs = data?.data?.docs || []

      let js_level_map = this.$config.js_level_map

      console.log(docs);
      this.attribuits_options = docs.map(item=>{
        let label = this.project_map[item.project] + ' / ' + js_level_map[item.level] + ' / ' + item.names?.zh_cn
    ...
    handle_img_description_record(item) {
      ...
      this.dialog_js_img_description = true;
    },

    async hadle_flush_imgdesc() {
      this.flush_imgdesc_loading = true
      try {
        await api_imgDescription.handle_flush({key_type: KEY_TYPE_JS_KEY})
      } catch (error) {}
      this.$lodash.delay(() => {
