<template>
  <q-card class="my-card mw50vw q-px-md w50vw">
    ...
    <q-card-section class="q-py-md q-gutter-y-md">
      <div class="row">
        <div class="w120 form-item-label">请求地址</div>
        <q-input class="col" filled dense v-model="editing_obj.url" />
      </div>
      <div class="row">
        <div class="w120 form-item-label">请求描述</div>
        <q-input class="col" filled dense v-model="editing_obj.desc" />
      </div>
      <div class="row">
        <div class="w120 form-item-label">code 码标签</div>
        <q-select class="col" filled dense emit-value map-options :options="code_tag_list" option-label="name"
          option-value="id" v-model="editing_obj.code_tag_id" />
        <q-btn color="purple" class="q-ml-md" @click="to_create_new_tag" label="新建标签" />
      </div>
      <div class="row">
...
export default {
  ...
  methods: {
     ...
     */
    handle_conform_edit() {
      let { id, url, desc, code_tag_id, status, mark } = this.editing_obj;
      let final_obj = { id, url, desc, code_tag_id, status, mark };
