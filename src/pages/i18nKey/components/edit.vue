  ...
  <q-card class="my-card mw50vw q-px-md w50vw">
    ...
    <q-card-section class="q-py-md q-gutter-y-md">
      ...
      <div class="row">
        ...
        <div class="col">
          <ValueI18nComponent  :detail_obj="detail_obj" @emit-edit="handle_emit_value"/>
        </div>
...
import ValueI18nComponent from "./value-i18n.vue";
...
export default {
  ...
  computed: {
    ...mapGetters(["get_all_layout","get_all_language", "get_all_keygroup_css"]),
    attribuits_options() {
      if (!this.editing_obj.project) return []
      return (this.get_all_keygroup_css || []).filter(ele => ele.project == this.editing_obj.project && ele.type == 1)
    }
  },
  watch: {
  ...
  methods: {
    ...
    handle_emit_value(obj){
      this.editing_obj.value = {...obj}
    },
    /**
     * 确定 提交更改
     *
