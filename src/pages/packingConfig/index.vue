 ...
 * @FilePath       : /client-api-doc-admin/src/pages/packingConfig/index.vue
 * @description    :
-->
<template>
  <div class="q-pa-md">
    ...
    <q-table class="sticky-header-table" :rows="table_data" :columns="columns" row-key="name"
      ...
      <template v-slot:body-cell-theme="props">
           ...
           <q-td :props="props">{{  props.row.theme.length  }} 个</q-td>
      </template>


      <template v-slot:body-cell-handle="props">
        <q-td :props="props">
          <q-btn color="primary" class="q-mr-md" glossy @click="handle_json_view_record(props.row)" label="查看JSON" />
   ...
   <!-- 编辑弹窗 -->
   <q-dialog v-model="show_dialog_JsonView" persistent>
      <JsonViewComponent :detail_obj="detail_obj"   />
    </q-dialog>
...
export default {
  ...
  data() {
    return {
      //API 命名空间
      api_namespace: api_packingConfig,
      //列表 表格项配置
      columns,
      //编辑弹窗内的数据
      detail_obj: {
      ...
      show_dialog_JsonView: false,
    };
  },
  methods: {
    handle_json_view_record(item, project) {
      this.detail_obj = item;
      this.show_dialog_JsonView = true;
    },
